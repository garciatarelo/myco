...
function Dashboard() {
  ...
  const calculateSoilProgress = ({ phValue, arsenicValue, areaValue }) => {
    const MAX_ARSENICO_NOM147 = 22;
    const phScore = Math.max(0, 100 - Math.abs(phValue - 7) * 20);
    const arsenicScore = Math.max(0, 100 - (arsenicValue / MAX_ARSENICO_NOM147) * 100);
    const areaBonus = Math.min(8, areaValue / 500);
    const fertilityScore = Math.round(Math.min(100, (phScore * 0.5) + (arsenicScore * 0.45) + areaBonus));

    return {
      fertilityScore,
      verdict: fertilityScore >= 70 ? 'Apto para sembrar' : fertilityScore >= 50 ? 'En recuperación' : 'Aún no apto',
    };
  };
  ...
  const markRobotAsOccupied = (terrainLabel) => {
    const freeIndex = robotFleet.findIndex((robot) => robot.estado === 'libre');
    if (freeIndex === -1) return false;

    setRobotFleet((previousFleet) => previousFleet.map((robot, index) => {
      if (index !== freeIndex) return robot;
      return {
        ...robot,
        estado: 'ocupado',
        tarea: terrainLabel,
  ...
  const handleAnalyze = async () => {
    if (!polygonCoords || area <= 0) {
      setTerrainAlertOpen(true);
      setAnalysisPhase('Falta dibujar el terreno');
      setAnalysisProgress(0);
      addLog(['[AVISO] Debes dibujar primero el terreno antes de analizar.']);
      return;
    }

    if (freeRobots.length === 0) {
      setAnalysisPhase('Ambos robots están ocupados');
      setAnalysisProgress(0);
      addLog(['[AVISO] Ambos robots están ocupados. No hay rover libre para analizar otro terreno.']);
      return;
    }
    ...
    try {
      ...
      if (polygonCoords && polygonCoords.length > 0) {
        try {
          ...
          setGeneratedRoute({ id: 'simulada', puntos_json: puntosSimulados, tiempo_estimado: 10 });
          setRouteStatus('planificada');
        } catch (e) {
          const puntosFallback = polygonCoords[0].map(coord => ({ lon: coord[0], lat: coord[1] }));
          setGeneratedRoute({ id: 'simulada', puntos_json: puntosFallback, tiempo_estimado: 10 });
          setRouteStatus('planificada');
        }
      }
    }
    ...
    const soilProgress = calculateSoilProgress({ phValue: phLevel, arsenicValue: arsenicLevel, areaValue: area });
    setSoilHistoryByMap((previousHistory) => {
      const currentHistory = previousHistory[activeMap] ?? [];
      const nextRecord = {
        id: Date.now(),
        session: currentHistory.length + 1,
      ...
      return {
        ...previousHistory,
        [activeMap]: [...currentHistory, nextRecord].slice(-12),
      };
    });
    addLog([`[SYS] Índice de fertilidad: ${soilProgress.fertilityScore}% (${soilProgress.verdict})`]);
  };

  const [logs, setLogs] = useState(['[SYS]   Sistema M.Y.C.O. iniciado.', '[SYS]   Válvula de micelio activa.', '[SYS]   Rover en campo...']);
  ...
  const startAutoCharge = () => {
    ...
    addLog(['[AVISO]  Batería crítica. Suspendiendo siembra...', '[AVISO]  Válvula cerrada. Rover regresando a base.', '[RUTA]   Ruta de regreso calculada. 480 m.', '[CARGA]  Rover en estación. Iniciando carga...']);
    chargeRef.current = setInterval(() => {
      setBatteryLevel(prev => {
        if (prev >= 95) {
          ...
          setValveOpen(true);
          setIsCharging(false);
          addLog(['[SYS]   Carga completa. Batería al 95%.', '[SYS]   Válvula reactivada. Rover al campo.']);
          return 95;
        }
        return prev + 2;
      });
  ...
  const card = { background: 'rgba(255,255,255,0.025)', border: '1px solid var(--line)', borderRadius: '8px', padding: '10px 12px' };
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '240px 1fr 360px', gap: '8px', height: '100%' }}>
        <aside className="panel" style={{ padding:'16px', display:'flex', flexDirection:'column', gap:'14px', overflow:'hidden' }}>
          ...
          <div>
            ...
            <div style={{ display:'flex', gap:'4px', marginTop:'4px' }}>
              {[{id:'mars',name:'Marte'},{id:'earth',name:'Chihuahua'}].map(m => (
                <button key={m.id} onClick={() => setActiveMap(m.id)} style={{ flex:1, border:'1px solid', borderColor: activeMap===m.id ? '#ff4500' : 'var(--line)', background: activeMap===m.id ? 'rgba(255,69,0,0.12)' : 'transparent', color: activeMap===m.id ? '#ff8a2b' : 'var(--muted)', borderRadius:'6px', padding:'6px 0', fontSize:'0.72rem', fontWeight: activeMap===m.id ? '700' : '400', cursor:'pointer', transition:'all 0.2s' }}>{m.name}</button>
              ))}
            </div>
          </div>
          <div style={{ ...card, display:'flex', flexDirection:'column', gap:'7px' }}>
            ...
            <div style={{ display:'flex', justifyContent:'space-between', fontSize:'0.74rem' }}>
              ...
              <strong style={{ color: isCharging ? '#ffc107' : failsafeStatus==='ACTIVE' ? '#ff7070' : '#5af7cf' }}>{isCharging ? 'Cargando...' : failsafeStatus==='ACTIVE' ? 'Volviendo a base' : 'En el campo'}</strong>
            </div>
            <div style={{ display:'flex', justifyContent:'space-between', fontSize:'0.74rem' }}>
              <span style={{ color:'var(--muted)' }}>Siembra</span>
              <strong style={{ color: valveOpen ? '#5af7cf' : '#ff7070' }}>{valveOpen ? 'Activa' : 'Pausada'}</strong>
            </div>
            <div>
          ...
          <div style={{ ...card, display:'flex', flexDirection:'column', gap:'8px', minHeight:'136px' }}>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:'8px' }}>
              {robotFleet.map((robot) => (
                <div key={robot.id} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', gap:'10px', fontSize:'0.72rem' }}>
                  <div style={{ display:'flex', flexDirection:'column' }}>
                    <strong style={{ color:'#fff', fontSize:'0.73rem' }}>{robot.nombre}</strong>
                    <span style={{ color:'var(--muted)', fontSize:'0.62rem' }}>{robot.tarea}</span>
                  </div>
                  <span style={{ padding:'3px 8px', borderRadius:'999px', border:'1px solid', borderColor: robot.estado === 'ocupado' ? '#ff7070' : '#5af7cf', color: robot.estado === 'ocupado' ? '#ff7070' : '#5af7cf', background: robot.estado === 'ocupado' ? 'rgba(255,112,112,0.12)' : 'rgba(74,235,183,0.08)', fontWeight:'700', textTransform:'uppercase', letterSpacing:'0.4px' }}>{robot.estado}</span>
                </div>
              ))}
            </div>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:'8px', overflowY:'auto', paddingRight:'4px' }}>
          <div className="panel" style={{ padding:'14px', flexShrink:0, display:'flex', flexDirection:'column', minHeight:'158px' }}>
            ...
            {!terrainAnalyzed ? <div style={{ display:'flex', alignItems:'center', justifyContent:'center', color:'var(--muted)', fontSize:'0.72rem', padding:'20px 0' }}>Sin terreno analizado</div> : (
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'7px' }}>
                {[ { l:'Temperatura', v:`${clima?.temp?.toFixed(1) ?? '—'} °C`, c: (clima?.temp??0)>0?'#ff8a2b':'#47d6ff' }, { l:'Humedad', v:`${clima?.hum ?? '—'}%`, c:'#5af7cf' }, { l:'Viento', v:`${clima?.viento ?? '—'} km/h`, c:'#b08cff' }, { l:'Lluvia', v:`${clima?.precip?.toFixed(1) ?? '—'} mm/h`, c:'#47d6ff' }, { l:'Presión', v:`${clima?.pres ?? '—'} hPa`, c:'#ffd24d' }, { l:'Condición', v: clima?.status ?? '—', c:'#ff8a2b' } ].map(({ l, v, c }) => (
                  <div key={l} style={{ ...card, display:'flex', flexDirection:'column', justifyContent:'center' }}><span style={lbl}>{l}</span><span style={{ fontSize:'0.95rem', fontWeight:'700', color:c, wordBreak:'break-word', lineHeight:1.3 }}>{v}</span></div>
                ))}
              </div>
            )}
          </div>
          <div className="panel" style={{ padding:'14px', flexShrink:0, display:'flex', flexDirection:'column', minHeight:'176px' }}>
            ...
            {!terrainAnalyzed ? <div style={{ display:'flex', alignItems:'center', justifyContent:'center', color:'var(--muted)', fontSize:'0.72rem', textAlign:'center', padding:'20px 0' }}>Sin terreno analizado</div> : (
              <div style={{ display:'flex', flexDirection:'column', gap:'14px' }}>
                <div><div style={{ display:'flex', justifyContent:'space-between', fontSize:'0.74rem', marginBottom:'5px' }}><span style={{ color:'var(--muted)' }}>Acidez (pH)</span><strong style={{ color: phLevel<6||phLevel>8 ? '#ff7070' : '#5af7cf' }}>{phLevel.toFixed(1)}</strong></div><input type="range" min="0" max="14" step="0.1" value={phLevel} onChange={e => setPhLevel(+e.target.value)} disabled={failsafeStatus==='ACTIVE'} style={{ width:'100%' }} /><span style={{ fontSize:'0.62rem', color: phLevel<6||phLevel>8 ? '#ff7070':'var(--muted)', display:'block', marginTop:'3px' }}>{phLevel<6 ? 'Suelo ácido' : phLevel>8 ? 'Suelo alcalino' : 'Nivel ideal (6–8)'}</span></div>
                <div><div style={{ display:'flex', justifyContent:'space-between', fontSize:'0.74rem', marginBottom:'5px' }}><span style={{ color:'var(--muted)' }}>Arsénico</span><strong style={{ color: arsenicLevel>22 ? '#ff7070':'#5af7cf' }}>{arsenicLevel} mg/kg</strong></div><input type="range" min="0" max="100" value={arsenicLevel} onChange={e => setArsenicLevel(+e.target.value)} disabled={failsafeStatus==='ACTIVE'} style={{ width:'100%' }} /><span style={{ fontSize:'0.62rem', color: arsenicLevel>22 ? '#ff7070':'var(--muted)', display:'block', marginTop:'3px' }}>{arsenicLevel>22 ? 'Supera NOM-147 (máx 22)' : 'Dentro del límite'}</span></div>
                <div><div style={{ display:'flex', justifyContent:'space-between', fontSize:'0.74rem', marginBottom:'5px' }}><span style={{ color:'var(--muted)' }}>Humedad del suelo</span><strong style={{ color:'#47d6ff' }}>38%</strong></div><div style={{ height:'6px', borderRadius:'999px', background:'rgba(255,255,255,0.05)' }}><div style={{ width:'38%', height:'100%', borderRadius:'999px', background:'linear-gradient(90deg,#47d6ff,#5af7cf)' }} /></div></div>
              </div>
            )}
          </div>
          ...
          <div className="panel" style={{ padding:'14px', flexShrink:0, display:'flex', flexDirection:'column', gap:'10px' }}>
            ...
            <div ref={consoleRef} style={{ background:'#020305', border:'1px solid #141b27', borderRadius:'7px', padding:'8px 10px', fontFamily:'monospace', fontSize:'0.62rem', overflowY:'auto', display:'flex', flexDirection:'column', gap:'3px', height:'120px' }}><div style={{ color:'#2a3a4e', borderBottom:'1px solid #141b27', paddingBottom:'4px', marginBottom:'2px', fontSize:'0.58rem' }}>CONSOLA — ROVER-01</div>{logs.map((log, i) => { let c = '#85e89d'; if (log.includes('[AVISO]')) c = '#ff7b72'; if (log.includes('[SYS]')) c = '#79c0ff'; if (log.includes('[RUTA]')) c = '#ffa657'; if (log.includes('[CARGA]')) c = '#ffd580'; return <div key={i} style={{ color:c, lineHeight:1.5 }}>{log}</div>; })}</div>
          </div>
        </div>
      </div>
      ...
      {terrainAlertOpen && (
        <div className="modal-backdrop" role="dialog" aria-modal="true">
          <div className="modal panel">
            <h3>Falta dibujar el terreno</h3>
            <p className="state-msg">Primero selecciona un área en el mapa con la herramienta de dibujo. Cuando ya exista un terreno, podrás analizarlo y asignar un robot.</p>
            <div className="modal-actions">
              <button type="button" className="btn-primary" onClick={() => setTerrainAlertOpen(false)}>
                Entendido
              </button>
            </div>
          </div>
